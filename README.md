...
* multiple profiles, so differnt persons can use different packs


## Setup Server:

### Requirements:
* pubilc readable s3 bucket
* Docker
...
* HOMESERVER       the homeserver url, wich does render the preview images; Can be different from the server, where the sticker are saved
...
* upload the created `.json` of your stickerpack (located in `web/packs/`) to `/PROFILE_NAME/*.json` at your s3 bucket. 
  ⚠️ **Do not upload the `index.json`** The server does create this file live.
